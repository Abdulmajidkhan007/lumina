...
import { useFollowing } from '@/data/query/hooks/useFollowers';
...
export default function FollowingScreen(): React.JSX.Element {
  ...
  const { id: rawId } = useRoute<RouteProp<ProtectedStackParamList, 'Following'>>().params;
  const currentUser = useCurrentUser();

  const userId = parseUserId(rawId);
  const { data, isLoading, isError, refetch, fetchNextPage, hasNextPage, isFetchingNextPage } =
    useFollowing(userId as UserId);
  ...
  if (userId === null) {
    return (
      <SafeAreaView
        ...
        edges={['top']}
      >
        <NavHeader title="Following" onBack={goBack} theme={theme} />
  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        ...
        <View style={styles.body}>
          <ErrorState
            message="Couldn't load following."
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <EmptyState
              ...
              subtitle="Accounts they follow will appear here."
            />
          }
